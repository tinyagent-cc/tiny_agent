...
template<> class LLMModel<Mistral, embedding_tag>;

template<>
class LLMModel<Mistral, embedding_tag> {
    std::string       model_;
    EmbeddingConfig   config_;
    httplib::Client   client_;

    void init_client() {
        config_.log.debug("embeddings", "mistral client initializing (model=" + model_ + ")");
    ...
    EmbeddingResponse embed_raw(const std::vector<std::string>& texts) {
        ...
        log.debug("embeddings", "mistral embed (model=" + model_
        ...
        if (res->status != 200)
            throw APIError(res->status, "mistral API error: " + res->body);
        ...
        for (auto& item : data) {
            ...
            if (idx >= embeddings.size())
                throw Error("mistral embed: index " + std::to_string(idx)
...
public:
    ...
    LLMModel(std::string model, EmbeddingConfig cfg = {})
        ...
        , client_(config_.base_url.empty()
              ? "https://api.mistral.ai" : config_.base_url)
    { init_client(); }

    LLMModel(std::string model, std::string api_key)
        : LLMModel(std::move(model), EmbeddingConfig{.api_key = std::move(api_key)}) {}

    explicit LLMModel(ModelConfig cfg = {.model_name = "mistral-embed", .dimensions = 1024})
        : LLMModel(cfg.model_name.empty() ? "mistral-embed" : cfg.model_name,
...
using MistralEmbedding = LLMModel<Mistral, embedding_tag>;
static_assert(is_embedding<MistralEmbedding>, "MistralEmbedding must satisfy is_embedding");
