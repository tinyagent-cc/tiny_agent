...
template<> class LLMModel<Cohere, embedding_tag>;

template<>
class LLMModel<Cohere, embedding_tag> {
    std::string       model_;
    EmbeddingConfig   config_;
    httplib::Client   client_;

    void init_client() {
        config_.log.debug("embeddings", "cohere client initializing (model=" + model_ + ")");
    ...
    EmbeddingResponse embed_raw(const std::vector<std::string>& texts) {
        ...
        log.debug("embeddings", "cohere embed (model=" + model_
        ...
        if (res->status != 200)
            throw APIError(res->status, "cohere API error: " + res->body);

        auto parsed = json::parse(res->body);
        std::vector<std::vector<float>> embeddings;
        for (auto& vec : parsed["embeddings"]["float"])
            embeddings.push_back(vec.get<std::vector<float>>());
        return {std::move(embeddings), parsed.value("meta", json::object()), parsed};
    }
...
public:
    ...
    LLMModel(std::string model, EmbeddingConfig cfg = {})
        ...
        , client_(config_.base_url.empty()
              ? "https://api.cohere.com" : config_.base_url)
    { init_client(); }
...
using CohereEmbedding = LLMModel<Cohere, embedding_tag>;
static_assert(is_embedding<CohereEmbedding>, "CohereEmbedding must satisfy is_embedding");
